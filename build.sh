#!/bin/bash
echo "================================================"
echo " WetEQ VST3 Plugin - Build Script"
echo "================================================"
echo ""
...
if [ "$PLATFORM" = "macos" ]; then
    ...
    if [ -d "Release/WetEQ.vst3" ]; then
        echo "Plugin location: WetEQ/build/Release/WetEQ.vst3"
        echo ""
        
        echo "Step 3: Code signing for macOS..."
        echo "================================================"
        codesign --force --deep --sign - "Release/WetEQ.vst3"
        if [ $? -eq 0 ]; then
        ...
        if [ -f "../vst3sdk/build/bin/validator" ]; then
            ../vst3sdk/build/bin/validator "Release/WetEQ.vst3"
            if [ $? -eq 0 ]; then
                echo ""
    ...
    if [ -f "VST3/Release/WetEQ.vst3/Contents/x86_64-linux/WetEQ.so" ]; then
        echo "Plugin location: WetEQ/build/VST3/Release/WetEQ.vst3"
        echo ""
        
        echo "Step 3: Running VST3 validator..."
        echo "================================================"
        if [ -f "bin/Release/validator" ]; then
            bin/Release/validator "VST3/Release/WetEQ.vst3"
            if [ $? -eq 0 ]; then
                echo ""
                echo "Validation passed!"
            else
